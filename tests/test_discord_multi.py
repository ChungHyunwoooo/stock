...
class TestLoadWebhookUrls:
    def test_load_all_webhooks(self, tmp_discord_config):
        with patch("engine.notifications.alert_discord.CONFIG_PATH", tmp_discord_config):
            from engine.notifications.alert_discord import load_webhook_urls
    ...
    def test_load_fallback_to_default(self, tmp_discord_config_legacy):
        with patch("engine.notifications.alert_discord.CONFIG_PATH", tmp_discord_config_legacy):
            from engine.notifications.alert_discord import load_webhook_urls
            urls = load_webhook_urls()
    ...
    def test_load_empty_when_no_config(self, tmp_path):
        ...
        with patch("engine.notifications.alert_discord.CONFIG_PATH", config_path):
            from engine.notifications.alert_discord import load_webhook_urls
            urls = load_webhook_urls()
            assert urls == {}
...
class TestLoadWebhookUrlFor:
    def test_load_specific_channel(self, tmp_discord_config):
        with patch("engine.notifications.alert_discord.CONFIG_PATH", tmp_discord_config):
            from engine.notifications.alert_discord import load_webhook_url_for
...
class TestSaveWebhookUrls:
    def test_save_webhooks(self, tmp_path):
        ...
        with patch("engine.notifications.alert_discord.CONFIG_PATH", config_path):
            from engine.notifications.alert_discord import save_webhook_urls
...
class TestSendWithChannel:
    def test_send_signal_with_channel(self, tmp_discord_config):
        ...
        with patch("engine.notifications.alert_discord.CONFIG_PATH", tmp_discord_config), \
            ...
            from engine.notifications.alert_discord import send_signal
